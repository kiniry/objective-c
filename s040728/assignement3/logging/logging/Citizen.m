...
-(void) setParents:(NSMutableSet *)parents{
    ...
    if (MDebug && ![parents count] == 2) {
        NSLog(@"setParents: precondition failure: The parents should be exactly 2");
    }
    if ([parents count] == 2) {
        _parents = parents;
    }
...
-(void) setChildren:(NSMutableSet *)children{
    ...
    for (Citizen *c in children) {
        if (c.parents && ![c.parents containsObject:self]) {
            if (MDebug) {
                NSLog(@"setChildren :precondition failure: %@ has already parents",c.firstName);
            }
            _legal = false;
            break;
        }
